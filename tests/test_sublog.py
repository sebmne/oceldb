...
def test_sublog_event_types_catalog_helpers(ocel):
    ...
    assert sub.event_type_counts() == {"Pay Order": 2}
...
def test_sublog_object_types_filters_objects(ocel):
    sub = ocel.query.sublog(object_types=["order"])
    assert sorted(sub.object_ids()) == ["o1", "o2"]


def test_sublog_object_types_drops_orphan_events(ocel):
    # Restricting to 'customer': only e1 touches o3, so under the default
    # drop_orphan_events=True only e1 survives as an event touching any
    # surviving object.
    new = ocel.query.sublog(object_types=["customer"]).to_ocel()
    try:
        assert new.query.event_ids() == ["e1"]
    finally:
        new.close()
...
def test_sublog_object_types_keeps_orphans_when_disabled(ocel):
    new = (
        ocel.query
        .sublog(object_types=["customer"], drop_orphan_events=False)
        .to_ocel()
    )
    try:
        assert sorted(new.query.event_ids()) == ["e1", "e2", "e3", "e4", "e5"]
    finally:
        new.close()
...
def test_sublog_event_types_restricts_objects_via_materialization(ocel):
    # Only Pay Order events survive → e3 (→o1), e4 (→o2). Customer o3 isn't
    # touched by any Pay event so it should disappear from the materialized
    # log.
    new = ocel.query.sublog(event_types=["Pay Order"]).to_ocel()
    try:
        assert sorted(new.query.object_ids()) == ["o1", "o2"]
    finally:
        new.close()
...
def test_sublog_to_ocel_event_types_round_trip(ocel):
    new = ocel.query.sublog(event_types=["Pay Order"]).to_ocel()
    try:
        assert sorted(new.query.event_ids()) == ["e3", "e4"]
...
def test_sublog_to_ocel_object_types_drops_orphans(ocel):
    ...
    try:
        assert new.query.event_ids() == ["e1"]
        assert new.query.object_ids() == ["o3"]
        rows = new.sql(
            'SELECT ocel_event_id, ocel_object_id FROM "event_object"'
        ).fetchall()
        assert rows == [("e1", "o3")]
...
def test_sublog_to_ocel_identity_copies_dataset(ocel):
    ...
    try:
        assert sorted(new.query.event_ids()) == ["e1", "e2", "e3", "e4", "e5"]
        assert sorted(new.query.object_ids()) == ["o1", "o2", "o3"]
    finally:
        new.close()
